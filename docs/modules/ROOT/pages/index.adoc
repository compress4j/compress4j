...
* **Escaping symlinks.** Use `escapingSymlinkPolicy(DISALLOW)` to reject them, or `RELATIVIZE_ABSOLUTE` to rewrite
  absolute targets so they stay inside the output directory.
* **How far an archive may expand.** Use `maxEntries`, `maxEntrySize` and `maxTotalSize` to bound it. Breaching a limit
  throws `ArchiveLimitExceededException`, which an error handler cannot suppress.

[,java]
----
include::example$java/com/example/ReadmeExamples.java[tag=readme-extract-untrusted,indent=0]
----
