...
[source,bash]
----
./gradlew jreleaserDeploy --dryrun
----

This simulates the deployment without actually publishing anything. Use JReleaser's own `--dryrun` flag, not
Gradle's `--dry-run` — the latter skips every task action instead of running them in simulated mode.
